...
using Mapster;
using MudBlazor;
using PersonFit.Frontend.WebAssembly.Components.Dialogs;
using PersonFit.Frontend.WebAssembly.Shared;

namespace PersonFit.Frontend.WebAssembly.Components.EntityTable;
    ...
    where TRequest : new()
{
    ...
    private async Task<TableData<TEntity>> ServerReload(TableState state)
    {
        if (!Loading && Context.ServerContext is not null)
        {
            ...
            if (await ApiHelper.ExecuteCallGuardedAsync<PaginationResponse<TEntity>>(
    ...
    //
    //             if (await ApiHelper.ExecuteCallGuardedAsync<FileResponse>(
